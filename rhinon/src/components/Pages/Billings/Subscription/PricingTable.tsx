...
function PricingTableBody({ className, ...props }: React.ComponentProps<"tbody">) {
  return <tbody data-slot="table-body" className={cn("[&_tr]:divide-x [&_tr]:border-b", className)} {...props} />
}
...
function PricingTablePlan({
  ...
  isYearly,
  ...props
}: React.ComponentProps<"div"> & PricingPlanType & { isYearly: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 text-start relative h-16">
        <RollingPrice price={displayPrice} isYearly={isYearly} />
